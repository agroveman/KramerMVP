import { WorkflowType, type Employee } from "@prisma/client";
...
async function main() {
  ...
  const deptByCode: Record<string, (typeof departments)[number]> = Object.fromEntries(
    departments.map((d) => [d.code, d])
  ) as Record<string, (typeof departments)[number]>;
  ...
  function addEmployee(input: EmployeeSeed) {
  ...
  async function createDeptStandups(managerExternalId: string, teamPrefix: string, label: string) {
    const manager = employeeByExternalId.get(managerExternalId);
    if (!manager) return;
    const teamMembers = createdEmployees.filter((e: Employee) =>
      e.externalId.startsWith(teamPrefix)
    );
  ...
  async function createWorkflow(name: string, type: WorkflowType, approverExternalIds: string[]) {
    ...
    for (let i = 0; i < approverExternalIds.length; i++) {
      ...
      if (!approverExternalId) continue;
  ...
  for (let i = 0; i < 8; i++) {
    await createWorkflow(`Budget Request ${i + 1}`, WorkflowType.BUDGET, [
      "O020", // Ops head
  ...
  for (let i = 0; i < 5; i++) {
    await createWorkflow(`Vendor Contract ${i + 1}`, WorkflowType.VENDOR_CONTRACT, [
      "O020",
      controller.externalId,
    ]);
  }
  ...
  for (let i = 0; i < 5; i++) {
    await createWorkflow(`Production Launch ${i + 1}`, WorkflowType.PRODUCTION_LAUNCH, [
      "E020", // Eng mgr
      riley.externalId,
      vpEng.externalId,
    ]);
